...
import com.broadtech.spark.csv.util.TypeCast
...
import com.broadtech.spark.csv.readers.{BulkCsvReader, LineCsvReader}
import com.broadtech.spark.csv.util._
...
case class CsvRelation protected[spark] (
  ...
  private def tokenRdd(header: Array[String]): RDD[Array[String]] = {
    ...
    if (ParserLibs.isUnivocityLib(parserLib)) {
      ...
      val filterLine = if (useHeader) firstLine else null  //现在使用filterLines进行过滤

      baseRDD().mapPartitions { iter =>
        // When using header, any input line that equals firstLine is assumed to be header
        //---过滤掉第一行 这里我们过滤前3行
        val csvIter = if (useHeader) {
          iter.filter(!filterLines.contains(_))
        } else {
          iter
        }
        parseCSV(csvIter, csvFormat)  //这里进行csv的解析 按csvFormat进行
      }
    }
  }
    ...
    tokenRdd(schemaFields.map(_.name)).flatMap { tokens =>
      ...
      if (dropMalformed && schemaFields.length != tokens.length) {
        ...
        try {
          ...
          val csvcolumMaps=firstLine.split(delimiter).zipWithIndex.map(x=>(x._1,x._2)).toMap
          index = 0
          while (index < schemaFields.length) {
            val field = schemaFields(index)
            if(csvcolumMaps.contains(field.name)){
              val num=csvcolumMaps.get(field.name).get
              rowArray(index) = TypeCast.castTo(tokens(num), field.dataType, field.nullable,
...
//          }
          ...
          case aiob: ArrayIndexOutOfBoundsException if permissive =>
            (index until schemaFields.length).foreach(ind => rowArray(ind) = null)  //多余的补空
            Some(Row.fromSeq(rowArray))
   ...
   * Returns the first line of the first non-empty file in path
    * 获取 firstLine 我们这里改成第三行
   */
  private var filterLines:Array[String]=_          // 通常是第一行 这里我们是前三行
  private lazy val firstLine:String = {
    if (comment != null) {
      filterLines=baseRDD().filter { line =>
        line.trim.nonEmpty && !line.startsWith(comment.toString)
      }.take(3)
      //filterLines(2).replace(",",this.delimiter.toString)
      filterLines(2).split(",").map(_.split(":")(1)).reduce((x,y)=>x+this.delimiter+y)
    } else {
      filterLines=baseRDD().filter { line =>
        line.trim.nonEmpty
      }.take(3)
  ...
  private def parseCSV(
    ...
    iter.flatMap { line =>
      try {
        val records: java.util.List[CSVRecord] = CSVParser.parse(line, csvFormat).getRecords
        //val aa=records.head.toArray
        if (records.isEmpty) {
